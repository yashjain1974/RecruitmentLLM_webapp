...
class BabyAGI(Chain,BaseModel):
    ...
    def print_task_list(self) -> str:
        output = io.StringIO()
        output.write("\n*****TASK LIST*****\n")
        lis.append("\n*****TASK LIST*****\n")
        for t in self.task_list:
            output.write(str(t["task_id"]) + ": " + t["task_name"] + "\n")
            lis.append(str(t["task_id"]) + ": " + t["task_name"] + "\n")
        return output.getvalue()

    def print_next_task(self, task: Dict) -> str:
        output = io.StringIO()
        output.write("\n*****NEXT TASK*****\n")
        output.write(str(task["task_id"]) + ": " + task["task_name"] + "\n")
        lis.append("\n*****NEXT TASK*****\n"+str(task["task_id"]) + ": " + task["task_name"] + "\n")
        return output.getvalue()

    def print_task_result(self, result: str) -> str:
        output = io.StringIO()
        output.write("\n*****TASK RESULT*****\n" )
...
#     OBJECTIVE = "Find me the shoe companies which were profitable in last quarter"
